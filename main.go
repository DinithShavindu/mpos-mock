...
func main() {
	addr := flag.String("addr", "127.0.0.1:39103", "TCP listen address for MPOS mock")
	...
	if *httpAddr != "" {
		...
		go func() {
			log.Printf("HTTP API index listening on http://%s/ (GET returns JSON)", *httpAddr)
	...
	if err != nil {
		log.Fatalf("TCP listen %q: %v", *addr, err)
	}
	log.Printf("MPOS TCP mock listening on %s (latency=%v)", ln.Addr(), latency)
